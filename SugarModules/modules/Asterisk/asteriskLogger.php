       ...
       if( $buffer === FALSE )
			...
			if ($buffer == "\r\n") { // handle partial packets
				...
				if ($e['Event'] == 'Dial' && $e['SubEvent'] != 'End') //Asterisk Manager 1.1 returns 2 dial events with a begin and an end subevent, by excluding the subevent end we won't get multiple records for the same call.
					...
					else
					{
						...
						if (preg_match($asteriskMatchInternal, $eChannel) && !preg_match($asteriskMatchInternal, $eDestination)) {
							// SugarAsterisk recipient fix
							if (preg_match('#^(.+?)/(\d+)#i', $e['Dialstring'], $numSplit))
							{
								if (strlen($numSplit[2]) > 10 && strlen($numSplit[2]) < 15)
								{
									$recipientID = substr($numSplit[2], -10);
								}
								else
								{
							...
							// end
							$query         = sprintf("INSERT INTO asterisk_log (asterisk_id, asterisk_id2, call_record_id, channel, remote_channel, callstate, direction, CallerID, recipientID, timestampCall) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s',%s)", $e['DestUniqueID'], $e['UniqueID'], $callRecordId, $eChannel, $eDestination, 'NeedID', 'O', $tmpCallerID, $recipientID, 'FROM_UNIXTIME('.time().')');
        ...
        //                             }
                                ...
                                if( !empty($direction['contact_id']) ){
                                    ...
                                    // SAfix
                                    $assocAccount = findSugarAccountByPhoneNumber($rawData['recipientID']);
                                    // end
                                    if( $assocAccount != FALSE ) {
                                        ...
                                        // SAfix
                                        $assoSugarObject = findSugarObjectByPhoneNumber($rawData['recipientID']);
                                        // end
